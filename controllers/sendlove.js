  ...
  async.parallel(
    {
      getIntention: done => {
        request.get({ url: getUrl, json: true }, (err, req, body) => {
          ...
          if (req.statusCode !== 200) {
            req.flash("errors", {
              msg: `An error occured with status code ${req.statusCode}: ${
                req.body.message
              }`
            });
          }
      ...
      getLikes: done => {
        ...
        request.get(
          { url: getPartsUrl, qs: queryStringParts, json: true },
          (err, req, body) => {
            if (err) {
              return next(err);
        ...
        request.get(
          ...
          (err, req, body) => {
            ...
            for (var i = 0; i < commentsArray.length; i++) {
              // fix updatedAt
              commentsArray[i].updatedAt = commentsArray[i].updatedAt
                .toString()
                .substring(0, 10);
              // add userName
...
exports.postIntention = (req, res, next) => {
  ...
  /*
  validate the data entered by the user. TODO - go back and delete the image,
  ...
  */
  ...
  if (!mimeTypeActual.startsWith("image/")) {
    req.flash("errors", {
      msg: "Please upload an image of type GIF, JPG, or PNG :) "
    });
  ...
  if (fileSize > 10000000) {
    req.flash("errors", {
      msg: "Please upload an image smaller than 10 megs :) "
    });
    return res.redirect("/api/new_intention");
  }
  ...
  request(
    ...
    (err, request, body) => {
      ...
      if (request.statusCode !== 200) {
        req.flash("errors", {
          msg: `An error occured with status code ${request.statusCode}: ${
  ...
  request(
    ...
    (err, request, body) => {
      ...
      for (var i = 0; i < mapLocations.length; i++) {
        ...
        mapLocations[i]["description"] = mapLocations[i][
          "description"
        ].substring(0, 255);
        // fix updatedAt
        mapLocations[i].updatedAt = mapLocations[i].updatedAt
          .toString()
          .substring(0, 10);
        // add userName
...
exports.postDetail = (req, res, next) => {
  ...
  console.log(
    `posting  ${partType} detail for thingId ${thingId} and  personId ${personId}`
  );
  // check errors
  const errors = req.validationErrors();
  if (errors) {
